 ...
 *  cannot drift from the markup. Ported as-is from lumina-terminal. */
 ...
 *  `data-tauri-drag-region` attributes: the event target must be the
 *  attributed element itself, so double-clicking an interactive child
 *  (title-bar buttons, session rows) never toggles, exactly like
 *  single-click dragging skips them. */
