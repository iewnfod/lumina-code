import type {TranslationKey} from "./en-us.ts";

/**
 * Simplified Chinese. Partial on purpose: a key missing here renders in
 * English via the source table, so translations can land incrementally
 * without breaking the UI or the build.
 */
const zhCn: Partial<Record<TranslationKey, string>> = {
    "New Session": "新建会话",
    "Other Sessions": "其他会话",
    "Show more": "显示更多",
    "Show less": "收起",
    "Connecting to OpenCode...": "正在连接 OpenCode...",
    "Ask Lumina Code, use @ to add context, use / for commands": "向 Lumina Code 提问，使用 @ 添加上下文，使用 / 发送命令",
    "Connection error": "连接错误",
    "Send": "发送",
    "Stop": "停止",
    "Mode": "模式",
    "Model": "模型",
    "Thinking depth": "思考深度",
    "Build mode": "构建模式",
    "Off": "关闭",
    "Low": "低",
    "Medium": "中",
    "High": "高",
    "Max": "最高",
    "Project": "项目",
    "Default project directory": "默认项目目录",
    "Recent Projects": "最近的项目",
    "Browse...": "浏览...",
    "Choose This Folder": "选择此文件夹",
    "Add attachment": "添加附件",
    "Remove attachment": "移除附件",
    "No matching commands": "没有匹配的命令",
    "No matching files": "没有匹配的文件",
    // --- Transcript run footer ---
    "Copy": "复制",
    "Copied": "已复制",
    "Task duration": "任务持续时间",
    // --- Permission requests & questions ---
    "Permission request": "权限请求",
    "Access a folder outside the project": "访问项目外的文件夹",
    "Run a shell command": "运行 shell 命令",
    "Edit a file": "编辑文件",
    "Write a file": "写入文件",
    "Read files": "读取文件",
    "Fetch a web page": "抓取网页",
    "Search the web": "搜索网页",
    "Question": "提问",
    "Allow once": "仅此一次",
    "Always allow": "始终允许",
    "Reject": "拒绝",
    "Send answers": "发送回答",
    "Dismiss": "忽略",
    "Answer required": "必答",
    "Type your answer...": "输入你的回答...",
    "Previous": "上一个",
    "Next": "下一个",
    // --- Tool cards & thinking blocks ---
    "Shell": "终端",
    "Edit": "编辑",
    "Write": "写入",
    "and {n} more": "等 {n} 个文件",
    "Added": "已新增",
    "Modified": "已修改",
    "Deleted": "已删除",
    "Read": "读取",
    "Grep": "文本搜索",
    "Glob": "文件搜索",
    "List": "列出目录",
    "Todo": "待办",
    "Fetch": "抓取网页",
    "Search": "搜索网页",
    "Skill": "技能",
    "Subagent": "子代理",
    "Tool failed": "工具执行失败",
    "Subagent failed": "子代理执行失败",
    "Request failed": "请求失败",
    "Thinking...": "思考中...",
    "Thought process": "思考过程",
    "Working...": "进行中...",
    "tool call": "次工具调用",
    "tool calls": "次工具调用",
    "thought": "段思考",
    "thoughts": "段思考",
    // --- Settings ---
    "Language": "语言",
    "Follow System": "跟随系统",
    "Settings": "设置",
    "General": "通用",
    "About": "关于",
    "Appearance": "外观",
    "Light": "浅色",
    "Dark": "深色",
    "Window outline": "窗口描边",
    "Show a thin window edge when the desktop draws no shadow": "在无窗口阴影的桌面显示窗口边缘",
    "Whether the workspace activity panel collapses on outside clicks or stays open": "工作区动态面板是随外部点击收起，还是保持打开",
    "Auto collapse": "自动收起",
    "Always open": "总是打开",
    "Interface font": "界面字体",
    "Interface font size": "界面字号",
    "Code font": "代码字体",
    "Code font size": "代码字号",
    "Fonts": "字体",
    "Font size": "字号",
    "Decrease font size": "减小字号",
    "Increase font size": "增大字号",
    "Default": "默认",
    "Reset to default": "恢复默认",
    "OpenCode": "OpenCode",
    "Not connected": "未连接",
    "Dependencies": "依赖",
    // --- Session usage ring ---
    "Session usage": "会话用量",
    "Context": "上下文",
    "Input": "输入",
    "Output": "输出",
    "Reasoning": "思考",
    "Cache read": "缓存读",
    "Cache write": "缓存写",
    "Cache hit rate": "缓存命中率",
    "Session total": "会话累计",
    "Cost": "费用",
    // --- Transcript chrome ---
    "Untitled": "无标题",
    "Loading earlier messages...": "正在加载更早的消息...",
    "Loading conversation…": "正在加载对话…",
    "Scroll to load earlier messages": "滚动以加载更早的消息",
    "Waiting for OpenCode...": "等待 OpenCode...",
    "Send a message to start": "发送消息开始对话",
    "Delete session": "删除会话",
    "Yes": "是",
    "No": "否",
    // --- Model configuration ---
    "Close": "关闭",
    "Configure models...": "配置模型...",
    "No models configured": "没有配置任何模型",
    "Model settings": "模型配置",
    "Providers": "供应商",
    "Custom": "自定义",
    "Search providers...": "搜索供应商...",
    "No matching providers": "没有匹配的供应商",
    "Failed to load providers": "加载供应商失败",
    "Failed to load models": "加载模型失败",
    "Failed to load config": "加载配置失败",
    "Loading...": "加载中...",
    "Back": "返回",
    "Connected": "已连接",
    "Active": "当前使用",
    "Activate": "启用",
    "Remove": "移除",
    "Connect": "连接",
    "Connecting...": "连接中...",
    "API key": "API 密钥",
    "Paste your API key": "粘贴你的 API 密钥",
    "Or set environment variable": "或设置环境变量",
    "Waiting for authorization...": "等待浏览器授权...",
    "Authorization complete": "授权成功",
    "Authorization failed": "授权失败",
    "Authorization expired": "授权已过期",
    "Open in browser": "在浏览器中打开",
    "Cancel login": "取消登录",
    "Retry": "重试",
    "Cancel": "取消",
    "Add custom provider": "添加自定义供应商",
    "Edit custom provider": "编辑自定义供应商",
    "Provider ID": "供应商标识",
    "Display name": "显示名称",
    "Base URL": "接口地址（Base URL）",
    "Package": "npm 包名",
    "Models": "模型",
    "Enabled": "已启用",
    "Disabled": "已禁用",
    "Connect this provider to list its models": "连接此供应商后可管理其模型",
    "Add model": "添加模型",
    "Model ID": "模型 ID",
    "Model name": "模型名称",
    "Save": "保存",
    "Saving...": "保存中...",
    "Custom provider ID, Base URL and at least one model are required": "需要填写供应商标识、接口地址和至少一个模型",
    "The config file uses JSONC (comments) and cannot be edited here. Open it to edit manually.": "配置文件含注释（JSONC），无法在此自动修改，请打开文件手动编辑。",
    "Open config file": "打开配置文件",
    "No custom providers yet": "还没有自定义供应商",
    "Custom providers are stored in the global OpenCode config": "自定义供应商保存在 OpenCode 全局配置中",
    "Connect failed": "连接失败",
    "Save failed": "保存失败",
    // --- Workspace activity stats card ---
    "Workspace activity": "工作区动态",
    "Changes": "代码改动",
    "Terminals": "后台终端",
    "Terminal": "终端",
    "Subagents": "子代理",
    "Running": "运行中",
    "Finished": "已完成",
    "Failed": "失败",
    "Files": "个文件",
    "No changes yet": "暂无改动",
    "No messages yet": "暂无消息",
    "No output": "暂无输出",
    "Output unavailable": "输出不可用",
    "Collapse": "收起",
    // --- Plan workflow (approval card + stats section) ---
    "Plan progress": "计划进度",
    "Plan submission": "提交计划",
    "Task complete": "任务完成",
    "Plan amend": "修正计划",
    "Tasks": "个任务",
    "Approve plan": "批准计划",
    "Approve": "批准",
    "Invalid plan submission": "无效的计划提交",
    "Waiting for approval": "待批准",
    "Blocked": "已受阻",
    // --- Custom tools (Model settings → Tools) ---
    "Tools": "工具",
    "Vision": "识图",
    "Vision model": "识图模型",
    "Plan mode": "计划模式",
    "Adds a vision tool that text-only models can call to see images": "提供识图工具：纯文字模型可以调用它查看图片",
    "No vision-capable models found": "没有支持图片输入的模型",
    "Custom tools are stored in the global OpenCode config": "自定义工具保存在 OpenCode 全局配置中",
};

export default zhCn;
