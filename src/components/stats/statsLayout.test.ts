...
test("non-detail views never dock", () => {
    assert.deepEqual(planStatsLayout(3000, REM, false), {mode: "overlay", panelWidth: 0, laneWidth: 0});
});
...
test("narrow containers overlay (today's behavior)", () => {
    ...
    assert.equal(planStatsLayout(1067, REM, true).mode, "overlay");
});

test("the crossover docks at exactly the minimum panel width", () => {
    const plan = planStatsLayout(1068, REM, true);
...
test("medium containers clamp the panel to the budget", () => {
    const plan = planStatsLayout(1200, REM, true);
    assert.equal(plan.mode, "dock");
...
test("wide containers get the full 40rem panel", () => {
    const plan = planStatsLayout(1680, REM, true);
    assert.equal(plan.mode, "dock");
...
test("rem scaling moves the crossover with user zoom", () => {
    ...
    assert.equal(planStatsLayout(1327, 20, true).mode, "overlay");
    const plan = planStatsLayout(1328, 20, true);
    assert.equal(plan.mode, "dock");
    assert.equal(plan.panelWidth, 520);
});

test("degenerate rem input overlays rather than dividing by zero", () => {
    assert.equal(planStatsLayout(2000, 0, true).mode, "overlay");
});
